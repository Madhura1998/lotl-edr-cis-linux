...
Living-off-the-Land (LotL) attacks abuse legitimate operating-system utilities such as `bash`, `curl`, `wget`, `python3`, `cron`, and `ssh` to perform malicious activities.



Because these utilities are also routinely used by legitimate administrators, malicious activity can resemble normal administrative behavior and may be difficult to identify using traditional signature-based approaches.
